...
/**
 * @ClassName: QinkanSpiderProcessor
 * @Description: todo
 * @Company: 广州市两棵树网络科技有限公司
 * @Author: zhengcq
 * @Date: 2020/7/29
 */
public class QinkanSpiderProcessor implements PageProcessor {
    ...
    public void process(Page page) {
        ...
        for (String url : urlList) {
            String targetUrl = isTargetUrl(url);
            if (!targetUrl.equals("")) {
                if (TestSpider.targetUrlSet.contains(targetUrl)) {
                    continue;
                }
                TestSpider.targetUrlSet.add(targetUrl);
                TestSpider.targetUrlQueue.add(targetUrl);
            }
            ...
            String fileUrl = isFileUrl(url);
            if (!fileUrl.equals("")) {
                TestSpider.FileUrl  fileUrlVo = new TestSpider.FileUrl();
                fileUrlVo.setUrl(fileUrl);
                String name = page.getHtml().xpath("//div[@class='detail_right']/h1/text()").toString();
    ...
    private String isFileUrl(String url) {
        boolean fileFlag =  Pattern.matches(".*/d/mobi/.*$|.*/d/txt/.*$|.*/d/epub/.*$", url);
        if (fileFlag) {
           return url;
        } else  {
            return  "";
    ...
    private String dirUrl(String url) {
        boolean dirFlag = Pattern.matches("^/[a-zA-Z0-9]+/$|^/[a-zA-Z0-9]+/[a-zA-Z0-9]+$", url);
        if (dirFlag) {
            return "https://www.qinkan.net" + url + "index.html";
        } else {
            dirFlag = Pattern.matches("https://www.qinkan.net/.*/index[_0-9]+.html$", url);
            if (dirFlag) {
                return url;
            }
        }
