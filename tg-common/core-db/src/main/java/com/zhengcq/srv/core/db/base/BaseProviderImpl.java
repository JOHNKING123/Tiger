...
import com.zhengcq.core.util.InstanceUtil;
import com.zhengcq.core.utils.DataUtils;
...
public abstract class BaseProviderImpl<T extends BaseModel> implements BaseProvider<T> {
	...
	public static Page<Long> getPage(Map<String, Object> params) {
		...
		Integer size = 10;
		String orderBy = "id_";
		if (DataUtils.isNotEmpty(params.get("pageNum"))) {
			current = Integer.valueOf(params.get("pageNum").toString());
		}
		if (DataUtils.isNotEmpty(params.get("pageSize"))) {
			size = Integer.valueOf(params.get("pageSize").toString());
		}
		if (DataUtils.isNotEmpty(params.get("orderBy"))) {
		...
		try {
			...
			if (record.getId() == null) {
				...
				if (CacheUtil.getLock(lockKey)) {
					try {
						T org = dao.selectById(record.getId());
			...
			record = dao.selectById(record.getId());
	...
	public Page<T> query(Map<String, Object> params) {
		...
		page.setRecords(dao.selectIdPage(page, params));
		return getPage(page);
	}
	...
	public List<T> queryList(Map<String, Object> params) {
		List<Long> ids = dao.selectIdPage(params);
